...
class CorrelationOperateFpath:
    """
    Operate fpath for correlation.py
    """
    def GetObsPlace(self, fpath):
        obs_place_split = fpath.split("infs_")[1].split("_")
        obs_place = ""
        count = 0
        len_obs_place_split = len(obs_place_split)
        for element in obs_place_split:
            if count == len_obs_place_split - 1:
                obs_place =  obs_place + element + "_"
                break
            elif count == len_obs_place_split - 2:
                obs_place =  obs_place + element
                break
            else:
                obs_place =  obs_place + element + "_"
            count += 1
